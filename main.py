...
async def start(message: types.Message):
    await message.answer("Привет! Я школьный помощник.", reply_markup=get_keyboard())
...
async def today(message: types.Message):
    try:
        ...
        now_time = datetime.datetime.strptime(now, "%H:%M")
...
async def handle_all(message: types.Message):
    ...
    # Если это кнопка - игнорируем (уже обработано выше)
    if text in ["📅 Расписание", "➕ Урок", "📚 ДЗ", "➕ ДЗ", "🔔 Сегодня"]:
        return
